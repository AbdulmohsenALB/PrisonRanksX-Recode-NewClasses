...
public class InfinitePrestigeExecutor implements PrestigeExecutor {
	...
	private void setupAutoPrestige() {
		...
		autoPrestigeTask = BukkitTickBalancer.scheduleDistributedTask(this::silentPrestige,
				p -> !isAutoPrestigeEnabled(p), speed < 1 ? 1 : speed);
	...
	private void setupMaxPrestige() {
		...
		boolean plainNamePlaceholder = Messages.getPrestige().contains("%next_prestige%");
		SingleReplacementMessage fastPrestigeMsg = new SingleReplacementMessage(Messages.getPrestige(),
				plainNamePlaceholder ? "%next_prestige%" : "%nextprestige_display%");
		maxPrestigeTask = BukkitTickBalancer.scheduleConcurrentTask(player -> {
			...
			if (prestigeResult.isSuccessful() && !MAX_PRESTIGE_BREAKER.contains(uniqueId)) {
				...
				if (sendMsgContinuously)
					fastPrestigeMsg.send(player, plainNamePlaceholder ? prestigeName : prestige.getDisplayName());
				// Messages.sendMessage(player, Messages.getPrestige(), s ->
				// s.replace("%nextprestige%", prestigeName)
