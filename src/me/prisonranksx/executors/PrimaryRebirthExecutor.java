...
public class PrimaryRebirthExecutor implements RebirthExecutor {
	...
	public PrimaryRebirthExecutor(PrisonRanksX plugin) {
		...
		if (plugin.getGlobalSettings().isHologramsPlugin()) {
			...
			hologramDelay = plugin.getHologramSettings().getRebirthRemoveDelay() * 20;
		}
		// setupMaxRebirth();
		// setupAutoRebirth();
	}
	...
	public RebirthResult canRebirth(Player player, double balance, boolean skipLastRankCheck) {
		...
		String nextRebirthName = rebirth == null ? RebirthStorage.getFirstRebirthName() : rebirth.getNextRebirthName();
	...
	public void playSound(Player player) {
		XSound.Record soundRecord = plugin.getGlobalSettings().getRebirthSound();
	...
	public DistributedTask<Player> getAutoTask() {
		return autoRebirthTask;
	}

	@Override
	public ConcurrentTask<Player> getMaxTask() {
		return maxRebirthTask;
	}

	@Override
	public void stopTasks() {
		if (autoRebirthTask != null) {
			autoRebirthTask.getBukkitTask().cancel();
			autoRebirthTask = null;
		}
		if (maxRebirthTask != null) {
			maxRebirthTask.getBukkitTask().cancel();
			maxRebirthTask = null;
		}
	}
}
