	...
	public CompletableFuture<Map<UUID, User>> convert(UserControllerType type) {
		return CompletableFuture.supplyAsync(() -> {
			...
			if (type == UserControllerType.YAML) {
				FileBackup.fromFile(new File(plugin.getDataFolder(), "rankdata.yml"));
				FileBackup.fromFile(new File(plugin.getDataFolder(), "prestigedata.yml"));
				FileBackup.fromFile(new File(plugin.getDataFolder(), "rebirthdata.yml"));
				...
				File usersFile = new File(usersConfig.getDirectory());
