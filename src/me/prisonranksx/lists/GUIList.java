...
public abstract class GUIList {
	...
	public void setupActions() {
							...
							.updateInventory(p,
									StringManager
											.parsePlaceholders(
													GUIItemParser
															.formatPlaceholders(
																	e.getCurrentItem(), newTitle
																			.replace("%page%",
																					String.valueOf(playerPagedGUI
																							.getCurrentPage(p) + 1))
																			.replace("%last_page%", String.valueOf(
																					playerPagedGUI.getLastPage() + 1))
		...
		clickActionsFormatter.setupAction("switch-item", stringStack -> {
			return ItemStackParser.parse(stringStack);
		}, stack -> ClickAction.create(e -> {
			e.setCurrentItem(GUIItemParser.updateItemMetaFrom(stack.clone(), e.getCurrentItem()));
		}));
		clickActionsFormatter.setupAction("switch-item-temp", stringStack -> {
			...
			ItemStack originalStack = playerPagedGUI.pageInventories
					...
					.get(e.getSlot())
					.getItemStack();
			e.setCurrentItem(GUIItemParser.updateItemMetaFrom(entry.getKey().clone(), originalStack));
